...
exports.handler = async function (event) {
  ...
  try {
    ...
    const result = await genAI.models.generateContent({ model: modelName, generationConfig: { temperature: 0.8 }, contents: [{ parts: [{ text: prompt }] }] });
    ...
    return { statusCode: 200, headers: { "Content-Type": "application/json" }, body: JSON.stringify(randomWord) };
  } catch (error) {
    console.error("Error in get-word function:", error);
    return { statusCode: 500, body: JSON.stringify({ error: 'Could not fetch a new word.' }) };
  }
};