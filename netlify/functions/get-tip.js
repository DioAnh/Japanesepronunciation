...
exports.handler = async function (event) {
  ...
  try {
    ...
    const result = await genAI.models.generateContent({ model: modelName, contents: [{ parts: [{ text: prompt }] }] });
    const rawText = result.text;
    return { statusCode: 200, headers: { "Content-Type": "application/json" }, body: JSON.stringify({ tip: rawText }) };
  } catch (error) {
    console.error("Error in get-tip function:", error);
    return { statusCode: 500, body: JSON.stringify({ error: 'Could not fetch a tip.' }) };
  }
};